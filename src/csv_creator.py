...
PERM_HEADER = ["COD_ASIGNATURA", "ANIO_CARNET", "NUM_CARNET", "SIGLO", "ANIOP", "MESIP", "MESFP",
               "BLOQUE", "SECCION", "NRO_CREDITOS", "PERMISO", "RENGLON", "NOTA_ASIGNATURA", "IND_NOTA_SIN_EFECTO"]

GEN_HEADER = ["ANIO_CARNET", "NUM_CARNET", "SIGLO", "ANIOP", "MESIP", "MESFP", "SITUACION",
              "SITUACIONINSCRIP", "GENERAL", "LIMITE CR", "NOTA_X_CREDITO_PONDERADO", "TOTAL_CREDITOS", "PP"]

MEMO_HEADER = ["N°", "CARNÉ", "NOMBRES Y APELLIDOS.", "PERMISO", "PERIODO"]
...
class CsvCreator():
    ...
    def write_gen(self, student_id, general="", limite_cred="", pp=""):
        if len(student_id) == 6:
            student_id = '0' + student_id
        new_row = [student_id[0:2], student_id[2:], "1"  # Siglo
                   , self.anio, self.trim[0], self.trim[1], ""  # Situación
                   , ""  # Situación ins
                   , general  # General
    ...
    def write_perm(self, asig_code, student_id, num_cred=""):
        ...
        if len(student_id) == 6:
            student_id = '0' + student_id
        new_row = [asig_code, student_id[0:2], student_id[2:], "1"  # Siglo
                   , self.anio, self.trim[0], self.trim[1], ""  # Bloque
                   , ""  # Secci'on
