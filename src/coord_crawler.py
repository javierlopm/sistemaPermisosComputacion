...
class StudentDownloader():
    ...
    def search_student(self, student_id):
        ...
        if student_id:
            ...
            file = codecs.open("./" + self.save_dir + "/" +
                               student_id[0:2] + "-" + student_id[2:] + ".html", "w", encoding="iso-8859-1")
...
def get_all_classes(inf_acad, filtro=None):
    ...
    for i in range(len(codigo)):
        if filtro(codigo[i].text):
            ...
            all_classes.append("{} | {} | {} | {}".format(
                codigo[i].text, nombre[i].text, nota[i].text, trim))
...
def get_gen(inf_acad):
    ...
    def filtro(x): return patt.match(x) and not (x in ciclo_basico)

    return get_all_classes(inf_acad, filtro)


def get_elect(inf_acad):
    def filtro(x): return (x in area) or (x in libres)
    return get_all_classes(inf_acad, filtro)
