...
class PermStore():
    ...
    def insert_student(self, carnet, nombre, telefono, correo, indice, aprobados, comentario):
        c = self.conn.cursor()
        c.execute(std_insert, (carnet, nombre, telefono,
                               correo, indice, aprobados, comentario))
    ...
    def insert_perm(self, carnet, tipoPermiso, trim, anio, extra=None):
        ...
        if tipoPermiso == TipoPermiso.limite_creditos:
            c.execute(per_int, (carnet, tipoPermiso.value,
                                trim.value, anio, extra))
        elif tipoPermiso == TipoPermiso.pp:
            c.execute(per_int, (carnet, tipoPermiso.value,
                                trim.value, anio, extra))
        elif tipoPermiso == TipoPermiso.permiso_materia:
            c.execute(per_str, (carnet, tipoPermiso.value,
                                trim.value, anio, extra))
    ...
    def get_course_perms(self, materia, trimestre=None, anio=None):
        if trimestre:
            return self._run_with_args(extra_str_qry,  (TipoPermiso.permiso_materia.value, trimestre.value, anio, materia))
        else:
            return self._run_with_args(extra_str_qry_no_trim,  (TipoPermiso.permiso_materia.value, materia))
    ...
    def get_with_state(self, state, trimestre=None, anio=None):
        if trimestre:
            return self._run_with_args(state_qry,
                                       (state.value, trimestre.value, anio))
        else:
    ...
    def test(self):
        self.insert_student(1110552, "Javier López", "04129349938", "javierloplom@gmail.com", 4.12, 666,
        ...
        self.insert_student(1110584, "Carlos Martínez",
                            "0424222222", "1110584@usb.ve", 3.45, 432, "")
        self.insert_student(1110564, "Javier Ernesto",
                            "0424222222", "1110564@usb.ve", 5.0, 235, "")

        # Extra credito
        self.insert_perm(1110552, TipoPermiso.limite_creditos,
                         Trimestre.septiembreDiciembre, 2016, 18)

        self.insert_perm(1110552, TipoPermiso.permiso_materia,
                         Trimestre.septiembreDiciembre, 2016, "CI5438")

        self.insert_perm(1110584, TipoPermiso.permiso_materia,
                         Trimestre.septiembreDiciembre, 2016, "CI5438")

        self.insert_perm(1110552, TipoPermiso.permiso_materia,
                         Trimestre.septiembreDiciembre, 2016, "CI4722")
