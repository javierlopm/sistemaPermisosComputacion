...
import os
import getpass
from perm_store import PermStore
...
for sheet in gc.openall():
    ...
    for i,line in enumerate(sheet.get_worksheet(0).get_all_values()):
        ...
        if i>0:
            carnet = carnetToInt(line[1].split('@')[0])
            perm_storer.insert_student(carnet, "", line[4], line[3], line[15])
            for k in range(5,15):
                if line[k] != "":
                    if perms_dict[k] == 'm':
                        for elem in parseCoursesId(line[k], k):
                            perm_storer.insert_perm(carnet, 'm', trimestre_dict[line[2]], 0, elem)
                    if perms_dict[k] == 'l' or perms_dict[k] == 'p':
                        perm_storer.insert_perm(carnet, perms_dict[k], trimestre_dict[line[2]], 0, int(line[k]))
                    if perms_dict[k] == 'e' or perms_dict[k] == 'g':
                        perm_storer.insert_perm(carnet, perms_dict[k], trimestre_dict[line[2]], 0)
                    if perms_dict[k] == 'r':
                        for elem in parseCoursesId(line[k], k):
                            perm_storer.insert_perm(carnet, 'r', trimestre_dict[line[2]], 0, elem)
